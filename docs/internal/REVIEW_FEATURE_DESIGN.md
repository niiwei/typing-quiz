...
### 功能目标
为 敲脑壳 MindPop 项目添加基于 Anki SM-2 算法的间隔重复复习系统，提升用户长期记忆效果。
...
**现象**：`git pull` 报错 "untracked working tree files would be overwritten by merge"

**原因**：云服务器上有未跟踪的临时文件，与 GitHub 版本中的正式文件冲突

**解决方案**：
```bash
# 删除云端的冲突文件（保留 GitHub 版本为准）
