...
const answerSchema = z.object({
  ...
  parts: z.array(z.object({ segments: z.array(z.object({ kind: z.enum(['context', 'required']), text: z.string() })) })).optional(),
});
...
const groupInput = z.object({ name: z.string().min(1), description: z.string().optional(), displayOrder: z.number().int().optional(), quizIds: z.array(z.number().int()).optional() });
...
function register(server: McpServer, api: MindpopApi) {
  ...
  server.registerTool('import_quizzes', { description: '原子、幂等导入 TYPING 题库；同一逻辑重试必须复用 request_id', inputSchema: z.object({ request_id: z.string().uuid(), quizzes: z.array(quizInput).min(1) }), annotations: write }, async ({ request_id, quizzes }) => { try { const data = await api.request('POST', '/imports', { requestId: request_id, quizzes }); return result(data, `导入完成，共 ${(data as any).count ?? 0} 个题库`); } catch (e) { return failure(e); } });
  ...
  server.registerTool('delete_quiz', { description: '使用删除预览确认令牌删除题库', inputSchema: idInput.extend({ confirmation_token: z.string().min(1) }), annotations: destructive }, async ({ id, confirmation_token }) => { try { await api.request('DELETE', `/quizzes/${id}`, { confirmationToken: confirmation_token }); return result({ id, deleted: true }, `已删除题库 ${id}`); } catch (e) { return failure(e); } });
  server.registerTool('list_groups', { description: '列出当前用户分组', inputSchema: z.object({}), annotations: read }, async () => { try { const data = await api.request('GET', '/groups'); return result({ items: data }, `已找到 ${(data as any[]).length} 个分组`); } catch (e) { return failure(e); } });
  ...
  server.registerTool('delete_group', { description: '使用删除预览确认令牌删除分组（题库保留）', inputSchema: idInput.extend({ confirmation_token: z.string().min(1) }), annotations: destructive }, async ({ id, confirmation_token }) => { try { await api.request('DELETE', `/groups/${id}`, { confirmationToken: confirmation_token }); return result({ id, deleted: true }, `已删除分组 ${id}，题库仍保留`); } catch (e) { return failure(e); } });
  server.registerTool('add_quiz_to_group', { description: '将题库加入分组', inputSchema: z.object({ group_id: z.number().int().positive(), quiz_id: z.number().int().positive() }), annotations: write }, async ({ group_id, quiz_id }) => { try { const data = await api.request('POST', `/groups/${group_id}/quizzes/${quiz_id}`); return result(data, `已将题库 ${quiz_id} 加入分组 ${group_id}`); } catch (e) { return failure(e); } });
  server.registerTool('remove_quiz_from_group', { description: '从分组解除题库关联', inputSchema: z.object({ group_id: z.number().int().positive(), quiz_id: z.number().int().positive() }), annotations: write }, async ({ group_id, quiz_id }) => { try { const data = await api.request('DELETE', `/groups/${group_id}/quizzes/${quiz_id}`); return result(data, `已将题库 ${quiz_id} 从分组 ${group_id} 移除`); } catch (e) { return failure(e); } });
}
